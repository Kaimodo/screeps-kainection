...
export class CreepTools {
    ...
    public static harvest(creep: Creep): void {
        let tombstone = CreepTools.findClosestTombstoneWithEnergy(creep);
        if (tombstone && creep.withdraw(tombstone, RESOURCE_ENERGY) == OK) {
            return;
        }
        let ruin = CreepTools.findClosestRuinsWithEnergy(creep);
        if (ruin && creep.withdraw(ruin, RESOURCE_ENERGY) == OK) {
            return;
        }
        let container = CreepTools.findClosestContainerWithEnergy(creep);
        if (container && CreepTools.withdrawEnergyFromOrMoveTo(creep, container) != ERR_NO_PATH) {
            return;
        }
        let activeSource = CreepTools.findClosestActiveEnergySource(creep);
        if (activeSource && CreepTools.harvestEnergyFromOrMoveTo(creep, activeSource) != ERR_NO_PATH) {
            return;
        }
        if (tombstone && CreepTools.withdrawEnergyFromOrMoveTo(creep, tombstone) != ERR_NO_PATH) {
            return;
        }
        if (ruin && CreepTools.withdrawEnergyFromOrMoveTo(creep, ruin) != ERR_NO_PATH) {
            return;
        }
        let inactiveSource = CreepTools.findClosestEnergySource(creep);
        CreepTools.consoleLogIfWatched(creep, `moving to inactive source: ${inactiveSource?.pos.x},${inactiveSource?.pos.y}`);
        if (inactiveSource) {
        ...
        creep.say('🤔');
        CreepTools.consoleLogIfWatched(creep, `stumped. Just going to sit here.`);
    }
    ...
    public static withdrawEnergyFromOrMoveTo(creep: Creep, structure: Tombstone | Ruin | StructureContainer): ScreepsReturnCode {
        CreepTools.consoleLogIfWatched(creep, `moving to ${typeof structure}: ${structure.pos.x},${structure.pos.y}`);
    ...
    public static pickupFromOrMoveTo(creep: Creep, resource: Resource): ScreepsReturnCode {
        CreepTools.consoleLogIfWatched(creep, `moving to ${typeof resource}: ${resource.pos.x},${resource.pos.y}`);
    ...
    public static harvestEnergyFromOrMoveTo(creep: Creep, source: Source): ScreepsReturnCode {
        CreepTools.consoleLogIfWatched(creep, `moving to ${typeof source}: ${source.pos.x},${source.pos.y}`);
    ...
    public static stopWorkingIfEmpty(creep: Creep) {
        if (creep.memory.working && creep.store[RESOURCE_ENERGY] == 0) {
            CreepTools.consoleLogIfWatched(creep, 'stop working, empty');
    ...
    public static startWorkingIfFull(creep: Creep, message: string) {
        if (!creep.memory.working && creep.store.getFreeCapacity() == 0) {
            CreepTools.consoleLogIfWatched(creep, 'start working, full');
    ...
    public static workIfCloseToJobsite(creep: Creep, jobsite: RoomPosition, range = 3) {
        ...
        if (creep.store.getUsedCapacity() != 0 && creep.store.getFreeCapacity() != 0) {
            ...
            const source = CreepTools.findClosestActiveEnergySource(creep);
            ...
            CreepTools.consoleLogIfWatched(creep, `sourceCost: ${sourceCost}`);
            ...
            CreepTools.consoleLogIfWatched(creep, `runCost: ${runCost}, refillEfficiency: ${refillEfficiency}`);
            ...
            CreepTools.consoleLogIfWatched(creep, `jobsiteCost: ${jobsiteCost}, storeRatio: ${storeRatio}, jobsiteEfficiency: ${jobsiteEfficiency}`);

            // compare cost/energy delivered working vs refilling first
            if (jobsiteEfficiency < refillEfficiency) {
                CreepTools.consoleLogIfWatched(creep, `close to site: starting work`);
                creep.memory.working = true;
            }
            else {
                CreepTools.consoleLogIfWatched(creep, `close to source: stopping work`);
                creep.memory.working = false;
            }
        }
    }
    ...
    public static getRoadCostMatrix(roomName: string): CostMatrix | boolean {
        ...
        CreepTools.updateRoadCostMatrixForStructures(structures, cost);

        const constructionSites = room.find(FIND_CONSTRUCTION_SITES);
        CreepTools.updateRoadCostMatrixForStructures(constructionSites, cost);
