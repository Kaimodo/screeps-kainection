...
export class Spawner {
  ...
  public spawnCreeps() {
    ...
    private replaceOldHarvesters() {
        for (let i = 0; i < this.harvesters.length; i++) {
            ...
            if (!harvester.spawning && !harvester.memory.retiring == true) {
                ...
                const pathToReplace = CreepTools.getPath(this.spawn.pos, harvester.pos);
                const ticksToReplace = CreepTools.calcWalkTime(harvester, pathToReplace);
                CreepTools.consoleLogIfWatched(this.spawn, `harvester spawn: ticksToLive: ${harvester.ticksToLive}, ticksToSpawn: ${ticksToSpawn}, pathCost: ${ticksToReplace}`);
    ...
    private getMaxWorkerCount(): number {
        ...
        if (this.rcl <= 1) {
            CreepTools.consoleLogIfWatched(this.spawn, `low rcl, max workers: ${config.MAX_WORKERS}`);
        ...
        if (conWork > 0) {
            ...
            CreepTools.consoleLogIfWatched(this.spawn, `work: ${conWork}, calc max workers: ${calculatedMaxWorkers}, final max workers: ${finalMaxWorkers}`);
